...
function loadUserData() {
  ...
  if (username) {
    ...
    document.getElementById("points").textContent =
      savedPoints !== null ? savedPoints : 0;
    sdaCoins = savedSdaCoins !== null ? parseInt(savedSdaCoins, 10) : 0;
      ...
      if (isStrongPassword(password)) {
        ...
        if (savedUsername) {
          if (username === savedUsername) {
            ...
            alert(
              "An account already exists with the username " + savedUsername
            );
            alert(
...
function showDashboard(username) {
  let displayUsername = username.replace(/[^a-zA-Z]/g, "");

  document
    .querySelectorAll(".dashboard")
    .forEach((dashboard) => (dashboard.style.display = "none"));
...
document
  .getElementById("convertPointsButton")
...
document
  .getElementById("openGalleryButton")
  .addEventListener("click", function () {
